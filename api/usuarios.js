...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // POST — crear o reactivar usuario
        if (req.method === 'POST') {
            ...
            if (existe) {
                if (!existe.activo) {
                    // Si estaba eliminado, lo reactiva con los nuevos datos
                    await Usuario.findByIdAndUpdate(existe._id, {
                        activo: true,
                        pass: body.pass,
                        nombre: body.nombre || '',
                        rol: body.rol || 'Agente Remoto'
                    });
                    return res.status(201).json({ success: true, message: 'Usuario reactivado' });
                }
